...
export default class GameAuto extends React.Component {
  constructor() {
    super();
    this.state = {
  ...
  applyMove(move) {
    // Parse the move string (assuming the format is 'P<fromSquare><toSquare>')
    if (move.charAt(3) === 'X') {
        const fromSquare = move.substring(1, 3);
        const toSquare = move.substring(4, 6);
  
        const fromIndex = this.state.place.indexOf(fromSquare);
        const toIndex = this.state.place.indexOf(toSquare);
  
        const squares = [...this.state.squares];
        let whiteFallenSoldiers = [...this.state.whiteFallenSoldiers];
        ...
        whiteFallenSoldiers.push({ ...squares[toIndex] });
      ...
      const fromSquare = move.substring(1, 3);
      const toSquare = move.substring(3, 5);
    
      // Find the index of the squares based on their names
      const fromIndex = this.state.place.indexOf(fromSquare);
      const toIndex = this.state.place.indexOf(toSquare);
  ...
  componentDidUpdate(prevProps, prevState) {
    ...
    if (this.state.val !== prevState.val) {
      if (this.state.counter === 0) {
        axios.post("http://localhost:5000/InsertWhiteBlack", this.state)
          .then(result => {
            console.log(result);
            if (result.data && result.data.data) {
              // Update the state with the updated data received from the server
              this.setState({
              tableData: [...this.state.tableData, result.data.data],
               counter: 0,
              sno: this.state.sno + 1,
              });
              console.log(result.data.data.black)
              this.applyMove(result.data.data.black);
              } else {
